...
def ringmod(*pitches: _Pitch) -> list[Note]:
    """
...
def sumtones(*pitches: _Pitch) -> list[Note]:
    """
...
class Difftone:  
    ...
    def __init__(self, note0:_Pitch, note1:_Pitch, desired:_Pitch=None):
...
def difftones(*pitches: _Pitch) -> list[Note]:
    """
...
def difftones_cubic(*notes:_Pitch) -> list[Note]:
    """
...
def difftone_sources_in_range(difftone: _Pitch, minnote:_Pitch=None,
    maxnote:_Pitch="D8", resolution=0.5, maxinterval=7.) -> list[Difftone]:
    """
...
def _difftone_find_source(pitch, maxdeviation=0.5, intervals=None,
                          minnote:_Pitch='A0', maxnote:_Pitch='C8', resolution=1.
...
def _sumtone_find_source(pitch, maxdist=0.5, intervals:list[U[int, float]]=None,
                         minnote:_Pitch='A0', maxnote:_Pitch='C8', difftonegap=0.):
    """
...
def difftone_sources_from_set(difftone: _Pitch,
                              sources: list[T],
...
def ringmod_exactsource(sideband1:_Pitch, sideband2:_Pitch) -> tuple[Note, Note]:
    """
...
def ringmod_sources2(sideband1:_Pitch, sideband2:_Pitch,
                     minnote:_Pitch="A0", maxnote:_Pitch="C8",
                     maxdiff=0.5
...
def difftones_beatings(pitch1: _Pitch, pitch2: _Pitch, maxbeatings=20, minbeatings=0.3, wave="saw",
                       report=True) -> list[DifftoneBeating]:
    """
...
def difftone_evaluate_inharmonicity(pitch1: _Pitch, pitch2: _Pitch) -> float:
    m1 = _asmidi(pitch1)
    m2 = _asmidi(pitch2)
    if m1 > m2:
        m1, m2 = m2, m1
    ...
    rat_2_d = f2/fd
    rat_1_d = f1/fd
    ratq_2_1 = lib.snap_to_grid(rat_2_1, 0.5)
    ratq_2_d = lib.snap_to_grid(rat_2_d, 0.5)
    ratq_1_d = lib.snap_to_grid(rat_1_d, 0.5)
