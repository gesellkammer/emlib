...
def makeAbjadVoice(notes: t.Seq[Note], grid="simple", glissUseMacros=True,
...
def quantizeAsAbjadRhythm(events: t.Seq[Event], grid="simple", annot=False, gliss=False
                          ) -> t.Tup[abj.Voice, t.Dict[int, Note]]:
    """
...
def _makeMusic21Voice(notes: t.Seq[Note], grid="simple") -> m21.stream.Stream:
    """
...
def makeMusic21Voice(notes: t.Seq[Note], grid="simple", divsPerSemitone=4, showcents=False):
...
def quantizeVoice(events: t.Seq[Event], grid="simple", divsPerSemitone=4,
                  showcents=False, showgliss=True, verify=True
                  ) -> m21.stream.Stream:
    """
    This is the center of this module: translate a series of non-overlapping scoring.Events
    (notes, chords) to a voice (a part) in musical notation (music21)

    :param events: a seq. of scoring.Events
    :param grid: the grid used to quantize the divisions of the beat. One of {'simple'}
    :param divsPerSemitone: the number of divisions per semitone (4 = 1/8 tones)
    :param showcents: show the cents deviation to the chromatic pitch under the note
    :param showgliss: generate glissando lines when indicated by the given Events
    :param verify: verify the quantization
...
def quantizeVoiceSplit(events: t.Seq[Event], grid="simple", divsPerSemitone=4,
                       showcents=False, showgliss=True,
                       ) -> t.List[m21.stream.Stream]:
    above, below = [], []
    for ev in events:
...
def _getEvent(m21note:m21.note.Note, mapping:t.Dict[int, Event]) -> t.Opt[Event]:
...
def _origEvent(m21obj:m21.note.NotRest) -> t.Opt[Event]:
...
def _partMakeIndex(part: m21.stream.Stream, mapping: t.Dict[int, Event], verify=True) -> None:
    """
    Add a reference to the original scoring note responsible for each m21 note
    within the note itself, as an editorial addition

    See quantizeVoice

    :param part: the part as returned by quantizeAsAbjadRhythm / abjadToMusic21
    :param mapping:
    :param verify:
    :return:
    """
    allevents = list(mapping.values())
    for note in part.flat.getElementsByClass('Note'):
...
def _mappedPartApplyPitches(part: m21.stream.Stream, divsPerSemitone=4,
                            showcents=False, inPlace=False) -> m21.stream.Stream:
    """

    :param part: the mapped part, where the pitches represent the indices to events in mapping
    :param divsPerSemitone:
    :param inPlace: operate on part itself, or create a new stream
