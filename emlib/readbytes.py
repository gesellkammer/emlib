...
def _num2bytes(n, typecode='i'):
    return struct.pack('='+typecode, n)


def _bytes2num(s, typecode):
    return struct.unpack('='+typecode, s)[0]
...
def read_sdif_1TRC(filename):
    #skip the headers:
    # load the first 300 hundred characters and find the first occurence
    # of 1TRC
    f = open(filename, 'rb')
    s = ''
    while 1:
    ...
    while f.tell() < end:
        ...
        if rowCount:
            frame = [read_x(f, datatype, columnCount) for i in xrange(rowCount)]
        else:
            frame = []
