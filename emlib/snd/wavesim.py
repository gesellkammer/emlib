...
class Instr:
    ...
    def rec(self, dur, outfile=None, sr=44100, ksmps=64, gain=1, block=True) -> str:
        ...
        outfile, popen = csound.recInstr(body=self.instrBody, init=self.instrInit, outfile=outfile,
    ...
    @lru_cache(maxsize=4)
    def chord(self, resolution=30, maxnotes=8, minamp=-50, maxfreq=12000, minfreq=0, t=-1):
        # type: (int, int, int, int, int, float) -> Chord
        """
        Returns the Chord at time `t`. The du
...
class Vowel(Instr):
    def __init__(self, midinote, vowel, gain=1, vibrate=5.3, vibamount=0, method="fof2"):
        ...
        super().__init__(name='emlib.wavesim.vowel',
...
class FreqMod(Instr):
    ...
    def __init__(self, freq, modmul, modindex=1, carmul=1.0, amp=1.0):
        ...
        self.carmul = carmul
        self.amp = amp
        super().__init__(name='emlib.freqmod',
                         instrBody=body,
                         instrInit="",
                         nchnls=1,
                         minDur=0.1,
...
class FreqMod2(Instr):
    def __init__(self, freq, carmul, modmul1, modmul2, idx1, idx2, amp=1.0):
        ...
        out aCar
        """
        super().__init__(name='emlib.freqmod2', instrBody=body, instrInit="", nchnls=1, minDur=0.1,
...
class FreqMod2Stacked(Instr):
    def __init__(self, freq, carmul, modmul1, modmul2, idx1, idx2, amp=1.0):
        ...
        out aCar
        """
        super().__init__(name='emlib.freqmod2stacked', instrBody=body, instrInit="", nchnls=1, minDur=0.1,
                         args=[amp, freq, carmul, modmul1, modmul2, idx1, idx2])

class RingMod(Instr):
    def __init__(selfs, freqs, amps, carrier, gain=1):
        """
        The carrier modulates an additive synthesis signal
        As a difference to RingModAll where all signals modulate each-other

        numSidebands = len(freqs) * 2

        a[n] = oscili(amp[n], freq[n]) * carrier * amp[n]
        asignal = sum(a)
        """
        pass
...
class RingModAll(Instr):
    def __init__(self, freqs, offsets=None, gain=1):
        ...
        super().__init__(name='emlib.ringmodx%d'%numosc, instrBody=body, instrInit="", nchnls=1, minDur=0.2)
...
class StaticSinesMono(Instr):
    ...
    def __init__(self, freqs, amps=None, gain=1):
        ...
        super().__init__(name='emlib.staticsinesmono%d'%numsines, instrBody=body, instrInit="", nchnls=1, minDur=0.2)
